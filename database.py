...
class DB:
    ...
    def init():
        with DB.conn() as c:
            c.execute('''CREATE TABLE IF NOT EXISTS users (
                ...
                games INTEGER DEFAULT 0, wins INTEGER DEFAULT 0, theme TEXT DEFAULT 'light',
                activity TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                created TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''')
            c.execute('''CREATE TABLE IF NOT EXISTS history (
                id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT, game TEXT,
                points INTEGER, won INTEGER, played TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    def register_user(user_id, name):
        with DB._lock, DB.conn() as c:
            c.execute('''INSERT INTO users (user_id, name, activity) VALUES (?, ?, CURRENT_TIMESTAMP)
                ON CONFLICT(user_id) DO UPDATE SET name = excluded.name, activity = CURRENT_TIMESTAMP''',
                (user_id, name))
    
    @staticmethod
    def get_user(user_id):
        with DB.conn() as c:
            row = c.execute('SELECT name, points, games, wins, theme FROM users WHERE user_id = ?',
                (user_id,)).fetchone()
    ...
    def add_points(user_id, points, won, game_type):
        with DB._lock, DB.conn() as c:
            c.execute('''UPDATE users SET points = points + ?, games = games + 1,
                wins = wins + ?, activity = CURRENT_TIMESTAMP WHERE user_id = ?''',
    ...
    def get_leaderboard(limit=20):
        with DB.conn() as c:
            rows = c.execute('''SELECT name, points, games, wins FROM users WHERE games > 0
                ORDER BY points DESC, wins DESC LIMIT ?''', (limit,)).fetchall()
