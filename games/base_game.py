...
class BaseGame(ABC):
    ...
    def handle_hint(self):
        if not self.current_answer:
            return None
        sample = self.current_answer[0] if isinstance(self.current_answer, list) else str(self.current_answer)
    ...
    def handle_withdrawal(self, user_id, display_name):
        ...
        return {"response": self.build_text_message(f"{display_name} انسحب من اللعبة"),
                "points": 0, "withdrawn": True, "game_over": True}

    def end_game(self):
        self.game_active = False
        c = self.get_theme_colors()
        if not self.scores:
            return {"response": self.build_text_message("انتهت اللعبة بدون نقاط"),
                    "points": 0, "game_over": True}
        ...
        contents = [
            ...
            {"type": "box", "layout": "vertical", "margin": "lg", "contents": [
                {"type": "text", "text": winner['name'], "size": "lg", "weight": "bold",
                 "color": c["text"], "align": "center"},
                {"type": "text", "text": f"{winner['score']} نقطة", "size": "xxl",
                 "weight": "bold", "color": c["success"], "align": "center", "margin": "sm"}
            ], "backgroundColor": c["card"], "cornerRadius": "12px", "paddingAll": "16px"}
        ]

        if len(sorted_players) > 1:
            contents.append({"type": "text", "text": "النتائج", "size": "sm", "weight": "bold",
                             "color": c["text"], "margin": "lg"})
            for i, (uid, data) in enumerate(sorted_players[:5]):
                contents.append({
                    "type": "box", "layout": "horizontal", "margin": "sm",
                    "contents": [
                        {"type": "text", "text": f"{i+1}", "size": "sm", "color": c["text2"], "flex": 0},
                        {"type": "text", "text": data['name'], "size": "sm", "color": c["text"],
                         "flex": 3, "margin": "sm"},
                        {"type": "text", "text": str(data['score']), "size": "sm", "weight": "bold",
                         "color": c["primary"], "align": "end", "flex": 1}
                    ]
                })
        ...
        bubble = {"type": "bubble", "size": "mega", "body": {"type": "box", "layout": "vertical",
                                                            ...
                                                            "backgroundColor": c["bg"]}}
        return {"response": FlexMessage(alt_text="نتيجة اللعبة", contents=FlexContainer.from_dict(bubble)),
                "points": winner['score'], "game_over": True, "won": True}
