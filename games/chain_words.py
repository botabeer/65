...
class ChainWordsGame(BaseGame):
    ...
    def start(self, group_id: str, players: dict) -> tuple[bool, str]:
        ...
        start_words = ["كتاب", "مدرسة", "سيارة", "بيت", "قلم", "شمس", "قمر", "نجم"]
        import random
        first_word = random.choice(start_words)
        
        self.used_words = {Config.normalize(first_word)}
        ...
        return True, f"لعبة سلسلة الكلمات بدأت\n\nالكلمة الأولى: {first_word}\nابدأ بكلمة تبدأ بحرف: {last_letter}"
    ...
    def process_message(self, group_id: str, user_id: str, message: str) -> tuple[str, bool]:
        ...
        normalized_word = Config.normalize(word)
        ...
        last_letter = Config.normalize(last_word[-1])
        first_letter = Config.normalize(word[0])
        ...
        next_letter = word[-1]
        return f"صحيح {player.display_name}\n\nالكلمة التالية يجب أن تبدأ بـ: {next_letter}", True
