        ...
        if self.game_phase == "joining":
            if normalized in ["انضم", "join"]:
                if user_id not in self.players and len(self.players) < self.max_players:
                    ...
                    return {
                        "response": self.build_text_message(f"{display_name} انضم - العدد: {len(self.players)}"),
                        "points": 0
                    }
                ...
                if len(self.players) >= self.min_players:
                    ...
                    return {
                        "response": self.build_text_message("بدأت اللعبة"),
                        "points": 0
                    }
                else:
                    return {
                        "response": self.build_text_message(f"يحتاج {self.min_players - len(self.players)} لاعبين اضافيين"),
                        "points": 0
                    }
        return None
