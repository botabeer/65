...
class MafiaGame(BaseGame):
    """لعبة المافيا بالعربي - كاملة مع نوافذ Flex وأزرار موحدة اللون"""
    BUTTON_COLOR = "#F2F2F7"
    ...
    def registration_message(self):
        ...
        player_list = [{"type": "text", "text": f"{i+1}. {p['name']}", "size": "sm", "color": c["text"], "margin": "xs" if i>0 else "md"} 
                       for i,p in enumerate(self.players.values())]
        if not player_list:
            player_list = [{"type": "text", "text": "لا يوجد لاعبين", "size": "sm", "color": c["text2"], "align": "center", "margin": "md"}]

        ready_text = f"{len(self.players)}/{self.min_players}" if len(self.players)<self.min_players else "جاهز"
        can_start = len(self.players) >= self.min_players
        start_color = c["accent"] if can_start else self.BUTTON_COLOR
        ...
        footer = {"type":"box","layout":"vertical","spacing":"sm","contents":[
            {"type":"button","action":{"type":"message","label":"انضم للعبة","text":"انضم مافيا"},"style":"primary","color":self.BUTTON_COLOR,"height":"sm"},
            {"type":"button","action":{"type":"message","label":"ابدأ اللعبة","text":"بدء مافيا"},"style":"primary","color":start_color,"height":"sm"},
            {"type":"button","action":{"type":"message","label":"إلغاء","text":"الغاء مافيا"},"style":"secondary","color":self.BUTTON_COLOR,"height":"sm"}
        ]}
    ...
    def assign_roles(self):
        ...
        return {"response":[self.build_text_message("تم توزيع الأدوار في الخاص"),self.night_message()]}

    def send_role_private(self,user_id,role):
        role_names={'mafia':'المافيا','detective':'المحقق','doctor':'الدكتور','citizen':'مواطن'}
        role_desc={'mafia':'دورك: اقتل شخص ليلا - اكتب: اقتل [الاسم]',
                   'detective':'دورك: افحص شخص ليلا - اكتب: افحص [الاسم]',
                   'doctor':'دورك: احمي شخص ليلا - اكتب: احمي [الاسم] او احمي نفسي',
                   'citizen':'دورك: صوت نهارا لطرد المافيا'}
        try:
            self.line_bot_api.push_message(PushMessageRequest(to=user_id,messages=[TextMessage(text=f"{role_names[role]}\n{role_desc[role]}")]))
    ...
    def night_message(self):
        ...
        alive_list=[{"type":"text","text":f"{i+1}. {p['name']}","size":"sm","color":c["text"],"margin":"xs" if i>0 else "md"} for i,p in enumerate(alive)]
        contents=[{"type":"text","text":f"الليل - اليوم {self.day_number}","size":"xl","weight":"bold","color":c["primary"],"align":"center"},
                  {"type":"separator","margin":"md","color":c["border"]},
    ...
    def day_message(self):
        ...
        alive=[p for p in self.players.values() if p['alive']]
        alive_list=[{"type":"text","text":f"{i+1}. {p['name']}","size":"sm","color":c["text"],"margin":"xs" if i>0 else "md"} for i,p in enumerate(alive)]
        contents=[{"type":"text","text":f"النهار - اليوم {self.day_number}","size":"xl","weight":"bold","color":c["primary"],"align":"center"},
                  {"type":"separator","margin":"md","color":c["border"]},
    ...
    def voting_message(self):
        c=self.get_theme_colors()
        alive=[p for p in self.players.values() if p['alive']]
        alive_buttons=[{"type":"button","action":{"type":"message","label":p["name"],"text":f"صوت {p['name']}"},"style":"primary","color":self.BUTTON_COLOR,"height":"sm"} for p in alive]
        vote_list=[]
        if self.votes:
        ...
        contents=[{"type":"text","text":"التصويت","size":"xl","weight":"bold","color":c["primary"],"align":"center"},
                  {"type":"separator","margin":"md","color":c["border"]},
                  {"type":"box","layout":"vertical","margin":"lg","contents":alive_buttons},
                  {"type":"separator","margin":"md","color":c["border"]},
    ...
    def process_night(self):
        ...
        result_text = f"انتهى الليل - اليوم {self.day_number}\n"
        ...
        if victim_id and victim_id != saved_id:
            ...
            result_text += "الدكتور أنقذ الضحية\n"
        else:
    ...
    def winner_message(self, winner_team):
        ...
        return FlexMessage(alt_text="انتهت اللعبة",contents=FlexContainer.from_dict(bubble))
    ...
    def status_message(self):
        ...
        alive_list=[{"type":"text","text":f"{i+1}. {p['name']}","size":"sm","color":c["text"],"margin":"xs" if i>0 else "md"} for i,p in enumerate(alive)]
        dead_list=[{"type":"text","text":f"{i+1}. {p['name']}","size":"sm","color":c["text2"],"margin":"xs" if i>0 else "md"} for i,p in enumerate(dead)]
        if not alive_list: alive_list=[{"type":"text","text":"لا يوجد","size":"sm","color":c["text2"],"margin":"md"}]
        if not dead_list: dead_list=[{"type":"text","text":"لا يوجد","size":"sm","color":c["text2"],"margin":"md"}]
    ...
    # ------------------ التعامل مع أوامر اللاعبين ------------------
    def check_answer(self,text,user_id,display_name):
        text=text.strip()
        if text=="انضم مافيا": return self.add_player(user_id,display_name)
        if text=="بدء مافيا": return self.assign_roles()
        if text=="حالة مافيا": return {"response":self.status_message()}
        if text=="الغاء مافيا":
            if self.phase=="registration":
                self.game_active=False
        ...
        if text=="انهاء الليل":
            if self.phase=="night": return self.process_night()
        ...
        if text=="تصويت مافيا":
            ...
            return {"response":self.build_text_message("ليس وقت التصويت")}
        if text.startswith("صوت "): return self.vote(user_id,text.replace("صوت ","").strip())
        if text=="انهاء التصويت":
            if self.phase=="voting": return self.end_voting()
            return {"response":self.build_text_message("ليس وقت التصويت")}
        if text.startswith("اقتل "):
            if user_id not in self.players or self.players[user_id]['role']!="mafia": return {"response":self.build_text_message("انت لست المافيا")}
            if self.phase!="night": return {"response":self.build_text_message("ليس وقت الليل")}
            target=text.replace("اقتل ","").strip()
            for uid,p in self.players.items():
                if p['name']==target and p['alive'] and uid!=user_id:
                    self.night_actions['mafia_target']=uid
                    return {"response":self.build_text_message(f"تم اختيار {target}")}
            return {"response":self.build_text_message("لا يوجد لاعب بهذا الاسم")}
        if text.startswith("افحص "):
            if user_id not in self.players or self.players[user_id]['role']!="detective": return {"response":self.build_text_message("انت لست المحقق")}
            if self.phase!="night": return {"response":self.build_text_message("ليس وقت الليل")}
            ...
            for uid,p in self.players.items():
                if p['name']==target and p['alive'] and uid!=user_id:
                    ...
                    return {"response":self.build_text_message(f"{target} هو {result}")}
            return {"response":self.build_text_message("لا يوجد لاعب بهذا الاسم")}
        if text.startswith("احمي "):
            if user_id not in self.players or self.players[user_id]['role']!="doctor": return {"response":self.build_text_message("انت لست الدكتور")}
            if self.phase!="night": return {"response":self.build_text_message("ليس وقت الليل")}
            ...
            for uid,p in self.players.items():
                if p['name']==target and p['alive']:
                    ...
                    return {"response":self.build_text_message(f"تم حماية {target}")}
            return {"response":self.build_text_message("لا يوجد لاعب بهذا الاسم")}
        return None
